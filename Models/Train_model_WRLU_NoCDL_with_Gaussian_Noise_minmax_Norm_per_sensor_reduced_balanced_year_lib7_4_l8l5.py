...
import re
BANDS_TO_USE = []
for band_ in ALL_BANDS:
    if re.match('.*BGR[01].*', band_):
        continue
    #if re.match('.*_TH_.*', band_):
    #    continue
    BANDS_TO_USE.append(band_)
...
FOLDER = '/' + 'project/nwisrl_water/nwisrl_water' + '/' + 'Irrigation_detection' + '/' + 'Irrigation_detection_WRLU_NoCDL_Balanced_Normalized_Unique_UT_lib7_4_l8l5'
...
geometry_augmentation = tf.keras.Sequential([
    layers.RandomFlip("horizontal_and_vertical"),
    layers.RandomZoom(0.2, fill_mode='reflect', fill_value=0, interpolation = 'nearest'),
...
MODELNAME = f'Balanced_Model_Irrig_detection_{output_type}_WRLU_{len(BANDS_TO_USE)}_bands_NoCDL_{KERNEL_SIZE}x{KERNEL_SIZE}_CONVSIZE{CONVSIZE}_{date_str}_{model_name}_{int(GAUSS_NOISE_VALUE*10000)}_D{int(DECAY_VALUE*100000) if DECAY_VALUE > 0 else "NoRegu"}'
print(MODELNAME)
import json
